...
void adc_init(void) {
	...
	struct adc_channel_config adcch_therm_conf;
	struct adc_channel_config adcch_pwr_conf;
	
	adc_read_configuration(&MY_ADC, &adc_conf);
	adcch_read_configuration(&MY_ADC, THERM_ADC_CH, &adcch_therm_conf);
	adcch_read_configuration(&MY_ADC, PWR_ADC_CH, &adcch_pwr_conf);
	
	adc_set_conversion_parameters(&adc_conf, ADC_SIGN_OFF, ADC_RES_12, ADC_REF_VCC);
	adc_set_conversion_trigger(&adc_conf, ADC_TRIG_MANUAL, 1, 0);
	adc_set_clock_rate(&adc_conf, 200000UL);
	adcch_set_input(&adcch_therm_conf, ADCCH_POS_PIN5, ADCCH_NEG_NONE, 1);
	adcch_set_input(&adcch_pwr_conf, ADCCH_POS_PIN3, ADCCH_NEG_NONE, 1);
	
	adc_write_configuration(&MY_ADC, &adc_conf);
	adcch_write_configuration(&MY_ADC, THERM_ADC_CH, &adcch_therm_conf);
	adcch_write_configuration(&MY_ADC, PWR_ADC_CH, &adcch_pwr_conf);
}
/*
...
float adc_get_therm_voltage(void){
	uint16_t adcReading;
	adc_enable(&MY_ADC);
	adc_start_conversion(&MY_ADC, THERM_ADC_CH);
	adc_wait_for_interrupt_flag(&MY_ADC, THERM_ADC_CH);
	adcReading = adc_get_result(&MY_ADC, THERM_ADC_CH);
	adc_disable(&MY_ADC);
	//printf("ADC reading = %u\n", adcReading);
...
float adc_get_pwr_voltage(void){
	uint16_t adcReading;
	adc_enable(&MY_ADC);
	adc_start_conversion(&MY_ADC, PWR_ADC_CH);
	adc_wait_for_interrupt_flag(&MY_ADC, PWR_ADC_CH);
	adcReading = adc_get_result(&MY_ADC, PWR_ADC_CH);
	adc_disable(&MY_ADC);
	//printf("ADC reading = %u\n", adcReading);
	//float voltage = 0.0009594 * (float)(adcReading);	//We have to find these numbers by applying differing voltage, printing ADC readings, and solve equation
